...
_b_ = [[194.9308796587644, 633.8682682974578, 991.9894930869377, 246.20781969209705, 602.159403132434, 818.1245523037857, 803.8545641799062, 831.8546635933892], [100.0, 863.3707580148779, 695.1690992740544, 567.1870392302833, 666.5441076472395, 835.6777643696155, 558.870955736481, 962.3196557625798], [186.0578241020075, 509.76665525248103, 528.8051412870569, 469.8803347544075, 761.1215389175708, 582.1754875635711, 688.7954353038984, 542.6480900412521], [421.60282643398864, 100, 469.8148603564696, 587.0673675234175, 138.62703661391362, 245.4508922259405, 569.5710982465911, 511.59472584213023], [272.3419031208225, 473.7448720966742, 374.799160037802, 769.7802347068023, 618.8238817877309, 543.9829450040743, 660.9598448433705, 553.5333689458909], [100.0, 334.5068106676668, 608.5374214096161, 754.5450313411786, 747.0026261957021, 499.25547973577045, 541.6117410631483, 578.7057182103983]]
...
def pic3():
    ...
    y1 = [0.1 * 0.01 * exp(0.0017 * x) / (0.1 + 0.01 * (exp(0.0017 * x) - 1)) for x in x]
    y2 = [0.1 * 0.01 * exp(0.00023 * x) / (0.1 + 0.01 * (exp(0.00023 * x) - 1)) for x in x]
    y3 = [0.1 * 0.01 * exp(0.00018 * x) / (0.1 + 0.01 * (exp(0.00018 * x) - 1)) for x in x]
    y4 = [0.1 * 0.01 * exp(0.00015 * x) / (0.1 + 0.01 * (exp(0.00015 * x) - 1)) for x in x]
    y5 = [0.1 * 0.01 * exp(0.00024 * x) / (0.1 + 0.01 * (exp(0.00024 * x) - 1)) for x in x]
    y6 = [0.1 * 0.01 * exp(0.00022 * x) / (0.1 + 0.01 * (exp(0.00022 * x) - 1)) for x in x]
    y7 = [0.1 * 0.01 * exp(0.00018 * x) / (0.1 + 0.01 * (exp(0.00018 * x) - 1)) for x in x]
    y8 = [0.1 * 0.01 * exp(0.00001 * x) / (0.1 + 0.01 * (exp(0.00001 * x) - 1)) for x in x]
...
44.019275723262545]
    ...
    print(1-sum(SO)/sum(totel))
    print(sum(totel) - sum(SO))
